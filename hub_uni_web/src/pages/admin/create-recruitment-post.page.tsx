...
export default function CreateRecruitmentPostPage() {
    ...
    const { data: professionData = [], isLoading: professionsLoading } = useGetProfessionsByOrganizationQuery(organizationId, { skip: !organizationId });
    const visaTypeOptions = useMemo(() => visaTypesData?.map((v: { Id: string; Name: string }) => ({ value: v.Id, label: v.Name })) ?? [], [visaTypesData]);
    const professionOptions = useMemo(() => professionData.map((p) => ({ Id: p.ProfessionId, Name: p.ProfessionName || "", })), [professionData]);
    ...
    const validate = () => {
        ...
        if (form.ProfessionIds.length === 0) newErrors.ProfessionIds = labels.professionRequired;
    ...
    return (
        <Box>
            ...
            <Grid container spacing={2}>
                <Grid size={12}>
                    <Paper elevation={1} sx={{ p: 2 }}>
                        <Grid container spacing={2.5}>
                            ...
                            <Grid size={{ xs: 12, sm: 6 }}>
                                <Autocomplete
                                    ...
                                    renderInput={(params) => (
                                        <TextField
                                            {...params}
                                            label={<>{labels.profession} <RequiredStar /></>}
                                            size="small"
                            ...
                            <Grid size={12} sx={{ mt: 1 }}><Typography sx={sectionLabelSx}>{labels.requirements}</Typography></Grid>
                            ...
                            <Grid size={12}>
                                <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 1 }}>
                                    <Typography variant="body2" fontWeight={600} color="text.secondary">{labels.otherReqs}</Typography>
                                    <Button size="small" startIcon={<Add />} onClick={handleAddOtherReq} sx={{ textTransform: "none", fontSize: 12 }}>{labels.add}</Button>
                                </Box>
                                {requirement.OtherReqs.length === 0 && (
                                    <Typography variant="body2" color="text.disabled" sx={{ fontStyle: "italic" }}>{labels.noOtherReqs}</Typography>
                                )}
                            ...
                            <Grid size={12} sx={{ mt: 1 }}><Typography sx={sectionLabelSx}>{labels.highlights}</Typography></Grid>
                            ...
                            <Grid size={{ xs: 12, sm: 6 }}>
                                <Box sx={{ display: "flex", gap: 1 }}>
                                    ...
                                    />
                                    <Button variant="outlined" onClick={handleAddHighlight} size="small">{labels.add}</Button>
                                </Box>
                            </Grid>
                            ...
                            <Grid size={{ xs: 12, sm: 6 }}>
                                <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1, minHeight: 36, alignItems: "center" }}>
                                    ...
                                    {form.Highlights.length === 0 && (
                                        <Typography variant="body2" color="text.secondary">{labels.noHighlights}</Typography>
                                    )}
                                </Box>
                            </Grid>
                ...
                <Grid size={12}>
                    <Paper elevation={1} sx={{ p: 2 }}>
                        <Typography sx={{ ...sectionLabelSx, mb: 1 }}>{<> {labels.description} <RequiredStar /></>}</Typography>

...
export default function PersonalInforPage() {
    ...
    if (isError || !data) return (<Box textAlign="center" py={8}><Typography color="error">{labels.cannotLoadUserInfo}</Typography></Box>);

    return (
        <Box>
            <Typography variant="h5" fontWeight={700} mb={3}>{labels.personalInformation}</Typography>
                ...
                <Grid size={{ xs: 12, md: 3 }}>
                    <Paper elevation={1} sx={{ p: 3, textAlign: "center", position: { md: "sticky" }, top: { md: 80 } }}>
                        <Box sx={{ position: "relative", display: "inline-block", mb: 2 }}>
                            ...
                            <Tooltip title={labels.updateAvatar}>
                                <Box
                        ...
                        <Stack spacing={1} alignItems="flex-start">
                            <Typography variant="caption" color="text.secondary" fontWeight={600}>{labels.status}</Typography>
                            <Chip
                                size="small"
                        ...
                        {data.AccountType && (
                            <Stack spacing={1} alignItems="flex-start" mt={2}>
                                <Typography variant="caption" color="text.secondary" fontWeight={600}>{labels.role}</Typography>
                        ...
                        {data.OrganizationName && (
                            <Stack spacing={1} alignItems="flex-start" mt={2}>
                                <Typography variant="caption" color="text.secondary" fontWeight={600}>{labels.organization}</Typography>
    ...
    return (
        <Box>
            <Stack direction="row" alignItems="center" justifyContent="space-between" mb={0.5}>
                <Typography variant="h6" fontWeight={700} fontSize="1rem">{labels.accountInformation}</Typography>
                <Stack direction="row" spacing={1} alignItems="center">
                    ...
                    {editing ? (
                        <Stack direction="row" spacing={1}>
                            <Button size="small" startIcon={<Cancel sx={{ fontSize: 16 }} />} onClick={handleCancel} sx={{ textTransform: "none", fontWeight: 600, fontSize: "0.8rem", color: "text.secondary" }}>{labels.cancel}</Button>
                            <Button variant="contained" size="small" startIcon={<Save sx={{ fontSize: 16 }} />} onClick={() => setConfirmOpen(true)} sx={{ textTransform: "none", fontWeight: 600, fontSize: "0.8rem", bgcolor: BLUE, "&:hover": { bgcolor: "#1975d1" } }}>{labels.update}</Button>
                        </Stack>
                    ) : (<Button variant="outlined" size="small" startIcon={<Edit sx={{ fontSize: 16 }} />} onClick={() => setIsEditing(true)} sx={{ textTransform: "none", fontWeight: 600, fontSize: "0.8rem", color: BLUE, borderColor: BLUE, "&:hover": { bgcolor: "#f2f4f7ff", borderColor: BLUE } }}>{labels.edit}</Button>)}
                </Stack>
            </Stack>
            <Divider sx={{ mb: 1 }} />
            <SectionLabel>{labels.basicInformation}</SectionLabel>
            <Grid container spacing={2}>
                <Grid size={{ xs: 12, sm: 6 }}>
                    <TextField size="small" fullWidth label={labels.fullName}
                ...
                <Grid size={{ xs: 12, sm: 6 }}>
                    <TextField size="small" fullWidth label={labels.userName}
                ...
                <Grid size={{ xs: 12, sm: 6 }}>
                    <TextField size="small" fullWidth label={labels.email} type="email"
                ...
                <Grid size={{ xs: 12, sm: 6 }}>
                    <TextField size="small" fullWidth label={labels.phoneNumber}
                ...
                <Grid size={{ xs: 12, sm: 6 }}>
                    <TextField size="small" fullWidth label={labels.dateOfBirth} type="date"
                ...
                <Grid size={{ xs: 12, sm: 6 }}>
                    <TextField select size="small" fullWidth label={labels.gender}
            ...
            <SectionLabel>{labels.address}</SectionLabel>
            <Grid container spacing={2}>
                <Grid size={{ xs: 12, sm: 6 }}>
                    <Autocomplete
                        ...
                        renderInput={(params) => (
                            <TextField {...params} label={labels.country} size="small" fullWidth sx={fieldSx(editing)}
                ...
                <Grid size={{ xs: 12, sm: 6 }}>
                    <Autocomplete
                        ...
                        renderInput={(params) => (
                            <TextField {...params} label={labels.province} size="small" fullWidth sx={fieldSx(editing)}
                ...
                <Grid size={{ xs: 12, sm: 6 }}>
                    <Autocomplete
                        ...
                        renderInput={(params) => (
                            <TextField {...params} label={labels.commune} size="small" fullWidth sx={fieldSx(editing)}
                                InputProps={{
                                    ...params.InputProps,
                ...
                <Grid size={{ xs: 12, sm: 6 }}>
                    <TextField size="small" fullWidth label={labels.addressDetail}
            ...
            <AdminConfirmDialog
                ...
                title={labels.confirmUpdate}
                message={labels.confirmUpdateMessage}
            />

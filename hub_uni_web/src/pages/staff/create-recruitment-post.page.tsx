import { Box, Button, Chip, CircularProgress, FormControl, FormControlLabel, FormHelperText, Grid, InputLabel, MenuItem, Paper, Select, Switch, TextField, Typography, Autocomplete, SelectChangeEvent } from "@mui/material";
...
export default function CreateRecruitmentPostPage() {
    ...
    const handleRequirementChange = useCallback((field: string, value: unknown) => { setRequirement(prev => ({ ...prev, [field]: value })); }, []);
    ...
    const handleRemoveHighlight = useCallback((index: number) => { setForm(prev => ({ ...prev, Highlights: prev.Highlights.filter((_, i) => i !== index) })); }, []);
    ...
    const handleSubmit = async () => {
        if (!validate()) return;
        try {
            await createRecruitmentPost({ ...form, Requirement: requirement, }).unwrap();
    ...
    return (
        <Box>
            <Box sx={{ display: "flex", alignItems: "center", mb: 3, gap: 1 }}>
                <Typography variant="h5" fontWeight={600}> Thêm bài đăng tuyển dụng </Typography>
            </Box>
            ...
            <Grid container spacing={2}>
                        ...
                        <Grid container spacing={2.5}>
                            ...
                            <Grid size={{ xs: 12, sm: 6 }}>
                                <FormControl fullWidth size="small" error={!!errors.ProvinceId}>
                                    ...
                                    >
                                        {provincesLoading ? (<MenuItem disabled><CircularProgress size={16} /></MenuItem>) : (provinces?.map(p => (<MenuItem key={p.Id} value={p.Id}>{p.Name}</MenuItem>)))}
                                    </Select>
                            ...
                            <Grid size={{ xs: 12, sm: 6 }} sx={{ display: "flex", alignItems: "center" }}>
                                <FormControlLabel
                                    control={<Switch checked={form.IsTop} onChange={e => handleChange("IsTop", e.target.checked)} color="primary" />}
                                    label="Nổi bật (Top)"
                                />
                            </Grid>
                            ...
                            <Grid size={{ xs: 12, sm: 6 }}>
                                <FormControl fullWidth size="small">
                                    ...
                                    >
                                        {Object.values(Gender).filter(v => typeof v === "number").map((g) => (<MenuItem key={g} value={g}>{ConvertService.convertGender(g as Gender)}</MenuItem>))}
                                    </Select>
                                </FormControl>
                            </Grid>
                            ...
                            <Grid size={{ xs: 12, sm: 6 }}>
                                <FormControl fullWidth size="small">
                                    ...
                                    >
                                        {Object.values(JobExperience).filter(v => typeof v === "number").map((exp) => (<MenuItem key={exp} value={exp}>{ConvertService.convertJobExperience(exp as JobExperience)}</MenuItem>))}
                                    </Select>
                                </FormControl>
                            </Grid>
                            ...
                            <Grid size={{ xs: 12, sm: 6 }}>
                                <FormControl fullWidth size="small">
                                    ...
                                    >
                                        {Object.values(EducationLevel).filter(v => typeof v === "number").map((edu) => (<MenuItem key={edu} value={edu}>{ConvertService.convertEducationLevel(edu as EducationLevel)}</MenuItem>))}
                                    </Select>
                                </FormControl>
                            </Grid>
                            ...
                            <Grid size={{ xs: 12, sm: 6 }}>
                                <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1, minHeight: 36, alignItems: "center" }}>
                                    ...
                                    {form.Highlights.length === 0 && (<Typography variant="body2" color="text.secondary"> Chưa có điểm nổi bật nào.</Typography>)}
                                </Box>
                            </Grid>
                ...
                <Grid size={12}>
                    <Paper elevation={1} sx={{ p: 2 }}>
                        <Typography sx={{ ...sectionLabelSx, mb: 1 }}>Mô tả công việc *</Typography>
